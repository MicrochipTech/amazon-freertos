...
/*-----------------------------------------------------------*/
                                     ...
                                     TCPIP_MAC_PACKET ** pPtrPkt )
    {
        ...
        if( pPkt != 0 )
        {
            ...
            TCPIP_MAC_PACKET ** ppkt = ( TCPIP_MAC_PACKET ** ) ( pBuff - PIC32C_BUFFER_PKT_PTR_OSSET );
    ...
    void NetworkBufferFree( uint8_t * pNetworkBuffer )
    {
        if( pNetworkBuffer != 0 )
        {
            TCPIP_MAC_PACKET ** ppkt = ( TCPIP_MAC_PACKET ** ) ( pNetworkBuffer - PIC32C_BUFFER_PKT_PTR_OSSET );
            configASSERT( ( ( uint32_t ) ppkt & ( sizeof( uint32_t ) - 1 ) ) == 0 );
